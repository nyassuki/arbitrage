...
// CoinEx API base URL
const API_BASE_URL = 'https://api.coinex.com';
...
async function getBalanceByAsset(asset) {
    try {
        const endpoint = `/v2/assets/financial/balance?ccy=${asset}`; // API endpoint for balance
        ...
        if (response.data.code === 0) {
            return {
                free: response.data.available, // Available balance
                locked: response.data.frozen, // Locked balance
            };
        } else {
            return response.data; // Return the error response
        }
...
//main();
async function main() {
	let address = await checkTradingPair("XMR", "USDT", "100");
	console.log(address);
}
